...
export default function PackageDetail() {
  ...
  if (!pkg) {
    return (
      <div style={{ minHeight: "100vh", background: "#0D1B2A", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "1rem" }}>
        <div style={{ fontSize: "4rem" }}>🔍</div>
        <div style={{ color: "white", fontWeight: 700, fontSize: "1.2rem" }}>{ar ? "الباقة غير موجودة" : "Package not found"}</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D1B2A", fontFamily: "Cairo, sans-serif" }}>

      {/* Back button */}
      <div style={{ position: "fixed", top: "80px", insetInlineStart: "1rem", zIndex: 100 }}>
        <button onClick={() => navigate("/")}
          style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.15)", color: "white", padding: "0.5rem 1rem", borderRadius: "50px", cursor: "pointer", fontWeight: 600, fontSize: "0.82rem", fontFamily: "Cairo, sans-serif", display: "flex", alignItems: "center", gap: "0.4rem" }}>
          {ar ? "← العودة" : "← Back"}
        </button>
      </div>
      ...
      <div style={{ position: "relative", height: "50vh", minHeight: "340px", overflow: "hidden" }}>
        ...
        {/* Image thumbnails */}
        <div style={{ position: "absolute", bottom: "1.5rem", insetInlineStart: "50%", transform: "translateX(-50%)", display: "flex", gap: "0.5rem" }}>
          {pkg.images.map((_, i) => (
            <button key={i} onClick={() => setActiveImg(i)}
              style={{ width: i === activeImg ? 28 : 8, height: 8, borderRadius: 4, background: i === activeImg ? pkg.color : "rgba(255,255,255,0.4)", border: "none", cursor: "pointer", transition: "all 0.3s" }} />
          ))}
        </div>

        {/* Hero text */}
        <div style={{ position: "absolute", bottom: "3.5rem", insetInlineStart: 0, insetInlineEnd: 0, padding: "0 2rem", textAlign: "center" }}>
          <div style={{ fontSize: "3.5rem", marginBottom: "0.5rem" }}>{pkg.icon}</div>
          <h1 style={{ fontSize: "2.2rem", fontWeight: 900, color: "white", margin: 0, lineHeight: 1.2 }}>{title}</h1>
        </div>
      </div>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "2rem 1.5rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "2rem", alignItems: "start" }}>
          ...
          <div style={{ position: "sticky", top: "90px" }}>
            <div style={{ background: `${pkg.color}0a`, border: `1px solid ${pkg.color}30`, borderRadius: "20px", padding: "1.5rem" }}>
              <div style={{ color: "#8899aa", fontSize: "0.75rem", marginBottom: "0.3rem" }}>{ar ? "السعر / فرد يبدأ من" : "Price / Person starts from"}</div>
              <div style={{ color: pkg.color, fontSize: "2rem", fontWeight: 900, fontFamily: "Montserrat, sans-serif", marginBottom: "0.25rem" }}>
                {formatPrice(pkg.priceEGP, currency, lang)}
              </div>
              <div style={{ color: "#667788", fontSize: "0.78rem", marginBottom: "1.5rem" }}>⏱ {duration}</div>

              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                <button onClick={() => { navigate("/"); setTimeout(() => { document.querySelector("#packages")?.scrollIntoView({ behavior: "smooth" }); }, 100); }}
                  style={{ background: `linear-gradient(135deg,${pkg.color},${pkg.color}cc)`, color: pkg.featured ? "#0D1B2A" : "white", border: "none", padding: "1rem", borderRadius: "14px", fontWeight: 800, fontSize: "0.95rem", cursor: "pointer", fontFamily: "Cairo, sans-serif", transition: "all 0.3s" }}
                ...
                <a href={`https://wa.me/201205756024?text=${waMsg}`} target="_blank" rel="noreferrer"
                  style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.3)", color: "#25D366", padding: "0.85rem", borderRadius: "12px", fontWeight: 700, fontSize: "0.88rem", textDecoration: "none", fontFamily: "Cairo, sans-serif" }}>
                  <WhatsAppIcon /> {ar ? "استفسار واتساب" : "WhatsApp Inquiry"}
                </a>
              </div>

              {/* Quick info */}
              <div style={{ marginTop: "1.5rem", paddingTop: "1.25rem", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "#8899aa", fontSize: "0.78rem" }}>
        ...
        {similar.length > 0 && (
          <div style={{ marginTop: "4rem" }}>
            <h2 style={{ color: "white", fontWeight: 700, fontSize: "1.2rem", marginBottom: "1.5rem" }}>{ar ? "رحلات مشابهة قد تعجبك" : "Similar Trips You May Like"}</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem" }}>
              {similar.map(sp => (
                <button key={sp.id} onClick={() => navigate(`/packages/${sp.slug}`)}
                  style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${sp.color}25`, borderRadius: "16px", padding: "1.25rem", cursor: "pointer", textAlign: "inherit", fontFamily: "Cairo, sans-serif", transition: "all 0.3s" }}
                  ...
                  <div style={{ fontSize: "2rem", marginBottom: "0.6rem" }}>{sp.icon}</div>
                  <div style={{ color: "white", fontWeight: 700, fontSize: "0.9rem", marginBottom: "0.35rem" }}>{ar ? sp.titleAr : sp.titleEn}</div>
                  <div style={{ color: sp.color, fontWeight: 800, fontSize: "0.95rem" }}>{formatPrice(sp.priceEGP, currency, lang)}</div>
                  <div style={{ color: "#667788", fontSize: "0.75rem", marginTop: "0.25rem" }}>⏱ {ar ? sp.durationAr : sp.durationEn}</div>
                </button>
              ))}
            </div>
          </div>
