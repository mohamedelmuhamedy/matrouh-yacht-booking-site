...
export default function CurrencySwitcher() {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", insetInlineEnd: 0, background: "var(--bg-surface-solid)", border: "1px solid var(--border-strong)", borderRadius: "12px", overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.5)", minWidth: "140px", zIndex: 1000 }}>
          {CURRENCY_LIST.map(c => {
            ...
            return (
              <button key={c} onClick={() => { setCurrency(c); setOpen(false); }}
                style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", background: active ? "rgba(0,170,255,0.1)" : "transparent", border: "none", padding: "0.7rem 1rem", cursor: "pointer", color: active ? "#00AAFF" : "var(--text-secondary)", fontFamily: "Cairo, sans-serif", fontSize: "0.82rem", textAlign: "inherit", transition: "background 0.15s" }}
